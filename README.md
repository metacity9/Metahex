...
|:---:|:---:|:---:|---|
| **Windows** | 실행 파일 (`.exe`) | [📥 Windows 다운로드](https://github.com/metacity9/Metahex/releases/latest/download/MetaHexEditor-windows.exe) | 다운로드 후 `MetaHexEditor-windows.exe` 더블 클릭 실행 |
| **Linux** | 실행 파일 (바이너리) | [📥 Linux 다운로드](https://github.com/metacity9/Metahex/releases/latest/download/MetaHexEditor-linux) | `chmod +x MetaHexEditor-linux` 실행 권한 부여 후 터미널에서 `./MetaHexEditor-linux` 실행 |
| **Python Source** | 파이썬 코드 (`.py`) | [📥 MetaHexEditor.py 다운로드](https://github.com/metacity9/Metahex/releases/latest/download/MetaHexEditor.py) | Python 3.x 설치 후 터미널에서 `python MetaHexEditor.py` 실행 |
...
* **직접 실행**:
  ```bash
  python MetaHexEditor.py
  ```
* **PyInstaller를 이용한 로컬 빌드**:
  ```bash
  pip install pyinstaller
  pyinstaller MetaHexEditor.spec
  ```
